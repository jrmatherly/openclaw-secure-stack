...
- `install.sh` generates `docker/egress/Corefile` from the allowlist (allowed domains forward to Cloudflare; all others get NXDOMAIN)
- To change the allowlist: edit `egress-allowlist.conf`, re-run `install.sh`, then rebuild with `docker compose build egress-dns`
...
| DNS resolution fails | CoreDNS not healthy | Check `docker compose logs egress-dns`; verify `docker/egress/Corefile` was generated by `install.sh` |
